...
resource "azurerm_api_management_product_policy" "openai_policy" {
  ...
  xml_content = replace(file("${path.module}/product-policy.xml"), "{tokens-per-minute}", 8)
}
...
resource "azurerm_api_management_api_policy" "apim_openai_policy" {
  ...
  xml_content = replace(file("${path.module}/policy.xml"), "{backend-id}", azapi_resource.apim_backend_pool_openai[0].name)
}
